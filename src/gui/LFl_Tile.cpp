...
// TODO: when draggin edges, make other widgets shrink to make room for
// more movement, e.g. serveral stacked widgets and the bottom one
// has room for shrinkage and the top one needs to be moved,
// shrink the bottom and shift down the weidget in between
//
// TODO: proper scaling, we need to make shure all widgets still have
// minimum size when the tab is resized
...
void LFl_Tile::position(int oix, int oiy, int newx, int newy) {
  ...
  for (int i=children(); i--; p += 4) {
    ...
    // if we would shrink a widget to nil, don't do it
    if (B == Y || R == X)
      return;
  }
