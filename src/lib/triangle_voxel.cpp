...
void TriangleVoxel::minimizePiece() {
...
bool TriangleVoxel::transform(unsigned int nr) {
...
bool TriangleVoxel::identicalInBB(const Voxel *op, bool includeColors) const {
  return (((bx1 + by1) & 1) == ((op->boundX1() + op->boundY1()) & 1))
      && Voxel::identicalInBB(op, includeColors);
}

void TriangleVoxel::transformPoint(int *x,
                                   int *y,
                                   int *z,
  ...
  double xpn = rotationMatrices[trans][0] * xs + rotationMatrices[trans][1] * ys
      + rotationMatrices[trans][2] * zs;
  double ypn = rotationMatrices[trans][3] * xs + rotationMatrices[trans][4] * ys
      + rotationMatrices[trans][5] * zs;
  double zpn = rotationMatrices[trans][6] * xs + rotationMatrices[trans][7] * ys
      + rotationMatrices[trans][8] * zs;
...
bool TriangleVoxel::getNeighbor(unsigned int idx,
                                unsigned int typ,
                                int x,
                                int y,
                                int z,
...
void TriangleVoxel::scale(unsigned int amount, bool grid) {
          ...
          for (unsigned int ay = 0; ay < amount; ay++)
            for (unsigned int az = 0; az < amount; az++) {
              ...
              if (!grid) {
                ...
                if (((az == 0 || az == amount - 1)
                    && (ay == 0 || ay == ax || ay + 1 == ax
                        ...
                        || ay == 2 * amount - 3 - ax))
                    || (ax == 0 && ay == 0)
                    || (ax == amount - 1 && ay == amount - 1)
                    || (ax == 2 * amount - 2 && ay == 0)
                    || (az == 0 && isEmpty2(x, y, z - 1))
                    || (az == amount - 1 && isEmpty2(x, y, z + 1))
                    || (ay == 0 && (((x + y) & 1) == 0 && isEmpty2(x, y - 1, z)
                        || ((x + y) & 1) == 1 && isEmpty2(x, y + 1, z)))
                    || ((ay == ax || ay + 1 == ax) && isEmpty2(x - 1, y, z))
                    || ((ay == 2 * amount - 2 - ax || ay == 2 * amount - 3 - ax)
                        && isEmpty2(x + 1, y, z))
                    )
                  value = get(x, y, z);
                else
              ...
              if ((ay <= ax) && (ay < 2 * amount - 1 - ax)) {
                if (((x + y) & 1) == 0) {
                  s2[(x * amount + ax)
                      + nsx * ((y * amount + ay) + nsy * (z * amount + az))] =
                      value;
                } else {
                  s2[(x * amount + ax) + nsx * ((y * amount + amount - 1 - ay)
                      + nsy * (z * amount + az))] = value;
                }
              }
            }
...
void TriangleVoxel::resizeInclude(int &px, int &py, int &pz) {
...
bool TriangleVoxel::validCoordinate(int /*x*/, int /*y*/, int /*z*/) const {

  return true;

}

bool TriangleVoxel::onGrid(int x, int y, int /*z*/) const {
...
void TriangleVoxel::getConnectionFace(int x,
                                      int y,
                                      int z,
                                      int n,
                                      double bevel,
    ...
    if (n < 15) {
      for (int i = 0; i < bevelFaces[p][n][0]; i++) {
        ...
        faceCorners.push_back(
            0.5 * x + vertices[v][0][0] + offset * vertices[v][1][0]
                + bevel * vertices[v][2][0]);
        faceCorners.push_back(
            HEIGHT * y + vertices[v][0][1] + offset * vertices[v][1][1]
...
void TriangleVoxel::calculateSize(float *x, float *y, float *z) const {
  *x = 1 + (getX() - 1) * 0.5;
  *y = getY() * HEIGHT;
  *z = getZ();
}

void TriangleVoxel::recalcSpaceCoordinates(float *x,
                                           float *y,
                                           float * /*z*/) const {
  *x = *x * 0.5;
  *y = *y * HEIGHT;
...
bool TriangleVoxel::meshParamsValid(double bevel, double offset) const {
