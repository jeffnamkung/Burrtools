 ...
 * don't make sense.
 */
class SphereVoxel : public Voxel {

 public:

  SphereVoxel(unsigned int x,
              unsigned int y,
              unsigned int z,
              const GridType *gt,
              voxel_type init = 0) : Voxel(x, y, z, gt, init) {}
  SphereVoxel(XmlParser &pars, const GridType *gt) : Voxel(pars, gt) {}
  SphereVoxel(const Voxel &orig) : Voxel(orig) {}
  SphereVoxel(const Voxel *orig) : Voxel(orig) {}
  ...
  virtual Polyhedron *getMesh(float sphere_rad,
 ...
 private:
  ...
  virtual Polyhedron *getMeshInternal(float sphere_rad,
                                      float connection_rad,
                                      float round,
                                      float offset,
                                      int recursion,
  ...
  void operator=(const SphereVoxel &);
};
