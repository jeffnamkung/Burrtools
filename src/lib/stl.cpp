...
void stlExporter_c::write(const char *fname,
                          const Voxel &v,
  ...
  if (binaryMode) {
    ...
    for (int i = 0; i < 84; i++) {
      if (fwrite(title + pos, 1, 1, f) != 1)
        throw stlException_c("Could not write file");
      if (title[pos]) pos++;
    }
  } else {
    f = fopen(fname, "w");
  ...
  try {
    ...
    if (!poly)
      throw stlException_c(
          "Something went wrong when generating the STL polyhedron");
  }
  ...
  for (Polyhedron::const_face_iterator it = poly->fBegin(); it != poly->fEnd();
       it++) {
    const Face *fc = *it;
    ...
    do {
      ...
      if (binaryMode) {
        // write normal vector
        if (fwrite(normal, 3, 4, f) != 4)
          throw stlException_c("Could not write file");

        // write the 3 vertices
        if (fwrite(v1, 3, 4, f) != 4)
          throw stlException_c("Coult not write file");
        if (fwrite(v2, 3, 4, f) != 4)
          throw stlException_c("Coult not write file");
        if (fwrite(v3, 3, 4, f) != 4)
          throw stlException_c("Coult not write file");
  ...
  if (binaryMode) {
    ...
    fseek(f, 80, SEEK_SET);
    if (fwrite(&triangleCount, 1, 4, f) != 4)
      throw stlException_c("Coult not write file");
  } else {
    fprintf(f, "endsolid\n");
  }
