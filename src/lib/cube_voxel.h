 ...
 */
class CubeVoxel : public Voxel {

 public:

  CubeVoxel(unsigned int x,
            unsigned int y,
            unsigned int z,
            const GridType *gt,
            voxel_type init = 0) : Voxel(x, y, z, gt, init) {}
  CubeVoxel(XmlParser &pars, const GridType *gt) : Voxel(pars, gt) {}
  CubeVoxel(const Voxel &orig) : Voxel(orig) {}
  CubeVoxel(const Voxel *orig) : Voxel(orig) {}
