...
class voxelTable_c {
 ...
 private:
  ...
  typedef struct hashNode {
    unsigned int
        index;           //< the shape index, which is given to the user to get the shape
    unsigned char
        transformation; //< which transformation of the shape is saved in here
    unsigned long
        hash;           //< the hash value of this transformation of the shape
 ...
 public:
   ...
   * is inside the table
   */
  bool getSpace(const Voxel *v,
                unsigned int *index = 0,
                unsigned char *trans = 0,
                unsigned int params = 0) const;
