...
typedef struct {
  ...
  float
      offset;        // offset by which the sphere radii are made smaller (subtraction)
  bool outside;        // is currently drawn sphere the outer or inner sphere
  float
      hole_diam;     // diameter of the hole between inside and outside > 0 round hole, <0 square hole
  float
      connection_rad;// radius of the connection between spheres (1 = maximal possible radius)
...
static float radius(float a, genPar &par) {
  ...
  float cp =
      par.curvX * par.curvX + par.curvY * par.curvY - par.curvRad * par.curvRad;
...
static void drawHolePiece(
  ...
  if (rec > 0 && fabs(start - end) > Epsilon) {
    ...
    findPointOnArc(connectionPoints[i][0],
    ...
    float l;
    l = sqrt(x1s * x1s + y1s * y1s + z1s * z1s);
    x1s *= rs / l;
    y1s *= rs / l;
    z1s *= rs / l;
    l = sqrt(x2s * x2s + y2s * y2s + z2s * z2s);
    x2s *= rs / l;
    y2s *= rs / l;
    z2s *= rs / l;

    l = sqrt(x1e * x1e + y1e * y1e + z1e * z1e);
    x1e *= re / l;
    y1e *= re / l;
    z1e *= re / l;
    l = sqrt(x2e * x2e + y2e * y2e + z2e * z2e);
    x2e *= re / l;
    y2e *= re / l;
    z2e *= re / l;
...
static void drawConnectingHole(
  ...
  if (rec > 0) {
    ...
    float diag23 = sqrt(
        (x2 - x3) * (x2 - x3) + (y2 - y3) * (y2 - y3) + (z2 - z3) * (z2 - z3));
    float diag14 = sqrt(
        (x1 - x4) * (x1 - x4) + (y1 - y4) * (y1 - y4) + (z1 - z4) * (z1 - z4));
...
static void makeSphere(uint16_t neighbors,
  ...
  for (int i = 0; i < 12; i++) {
    ...
    for (int t = 0; t < 6; t++) {
      ...
      if ((neighbors & (1 << i)) && par.outside
          && (par.connection_rad > Epsilon)) {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (!hollow || fabs(par.hole_diam) < Epsilon) { // not hollow or no holes
      for (int k = 0; k < 2; k++) {
        ...
        shiftToHoleBorder(squarePoints[i][(4 * k + 0) % 8][3],
                          &p1x,
                          &p1y,
                          &p1z);
        shiftToHoleBorder(squarePoints[i][(4 * k + 2) % 8][3],
                          &p2x,
                          &p2y,
                          &p2z);
        shiftToHoleBorder(squarePoints[i][(4 * k + 4) % 8][3],
                          &p3x,
                          &p3y,
                          &p3z);
      ...
      for (int k = 0; k < 4; k++) { //Once per side of the square
        ...
        float p1y = squarePoints[i][(2 * k + 0) % 8][1];
        float p1z = squarePoints[i][(2 * k + 0) % 8][2];
        shiftToHoleBorder(squarePoints[i][(2 * k + 0) % 8][3],
                          &p1x,
                          &p1y,
                          &p1z);
        ...
        shiftToHoleBorder(squarePoints[i][(2 * k + 2) % 8][3],
                          &p2x,
                          &p2y,
                          &p2z);
        ...
        shiftToConnectingHole(hx,
                              hy,
                              hz,
                              connectingHoleRadius,
                              &p3x,
                              &p3y,
                              &p3z);
        shiftToConnectingHole(hx,
...
static bool curvOk(float curvrad, float cnrad, float sprad, float offset) {
  ...
  float curvy = sqrt(
      (sprad - offset + curvrad) * (sprad - offset + curvrad) - curvx * curvx);
...
Polyhedron *SphereVoxel::getMeshInternal(float sphere_rad,
  ...
  par.curvY = sqrt(
      (sphere_rad - offset + par.curvRad) * (sphere_rad - offset + par.curvRad)
          - par.curvX * par.curvX);
  par.vl = &vl;
  par.holeStart = atan((par.connection_rad * (par.sphere_rad - par.offset) / 2)
                           / par.sphere_rad);
...
Polyhedron *SphereVoxel::getMesh(float sphere_rad,
                                 float connection_rad,
                                 float round,
                                 float offset,
                                 int recursion,
                                 float inner_rad,
                                 float hole_diam) const {
...
Polyhedron *SphereVoxel::getDrawingMesh(void) const {
  return getMeshInternal(
      0.5,  // sphere radius
...
Polyhedron *SphereVoxel::getWireframeMesh(void) const {
  return getDrawingMesh();
}

void SphereVoxel::getConnectionFace(int x,
                                    int y,
                                    int z,
                                    int n,
                                    double /*bevel*/,
...
void SphereVoxel::calculateSize(float *x, float *y, float *z) const {
  *x = 1 + (getX() - 1) * sqrt(0.5);
  *y = 1 + (getY() - 1) * sqrt(0.5);
  *z = 1 + (getZ() - 1) * sqrt(0.5);
}

void SphereVoxel::recalcSpaceCoordinates(float *x, float *y, float *z) const {
  *x = *x * sqrt(0.5);
  *y = *y * sqrt(0.5);
  *z = *z * sqrt(0.5);
}
